...
HtmlElement.prototype.toString = function() {
  ...
  let childrenFinal = "";
  let final = openingTag + this.text
  getChildren(this.children);
  return final + childrenFinal + closingTag;
  ...
  function getChildren(children) {
    for (let x in children) {
      const openingTag = getOpeningTag(children[x].type, children[x].id, children[x].classes);
      const closingTag = "</" + children[x].type + ">";
      childrenFinal += openingTag + children[x].text;
      let subChild = getChildren(children[x].children) || "";
      childrenFinal += closingTag;
    }
  }
};
...
// Hint: look up the splice method
